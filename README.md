...
- Global websocket frames without symbol binding are now classified separately as `ambient_global_only`. They are explicitly locked to diagnostic-only behavior by default: they stay out of symbol-scoped external context items, the live summary records that the feed was active, and we do not promote those frames into enrichment context unless we later add an explicit opt-in policy.
...
- Phase 8 starts the always-on operator workflow on top of those durable Phase 6 and Phase 7 artifacts. `tapescript_create_watch_definition`, `tapescript_list_watch_definitions`, and `tapescript_read_watch_definition` persist idempotent watch definitions for a portable case bundle plus analyzer profile, `tapescript_evaluate_watch_definition` turns a watch into a durable `phase8.trigger_run.v1` artifact that links the watch, source bundle, and reused-or-created Phase 7 analysis, and `tapescript_list_trigger_runs`, `tapescript_read_trigger_run`, and `tapescript_list_attention_items` expose the resulting trigger history and open operator inbox. The MCP resource surface mirrors that storage with `tape://phase8/watch/<id>`, `tape://phase8/watch/<id>/markdown`, `tape://phase8/trigger/<id>`, `tape://phase8/trigger/<id>/markdown`, and `tape://phase8/attention/open`, so agents can reopen raw JSON or a compact markdown summary without scraping manifests. TapeScope now has a compact native `Phase8Pane` for choosing a portable Phase 6 bundle, creating/evaluating a watch, browsing durable watch definitions, reviewing the current attention inbox, jumping into the linked Phase 7 analysis, and reopening the linked source artifact when a trigger needs deeper follow-up.
