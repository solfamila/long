...
void handleWebSocketOrder(const std::string& jsonMessage, ix::WebSocket& webSocket, TradingRuntime* runtime) {
    if (!runtime) {
        webSocket.send(json({{"success", false}, {"error", "Runtime not available"}}).dump());
        return;
    }
    ...
    try {
        ...
        int qtyShares = toShareCount(static_cast<double>(quantity));
        if (qtyShares <= 0) {
        ...
        std::uint64_t traceId = beginTradeTrace(intent);

        CommandResult result = runtime->submitOrder(symbol, action, qtyShares, limitPrice, closeOnly, "WebSocket", traceId);

        if (!result.success) {
            webSocket.send(json({{"success", false}, {"error", result.error}, {"traceId", static_cast<unsigned long long>(traceId)}}).dump());
            return;
        }

        webSocket.send(json({
            {"success", true},
            {"orderId", static_cast<long long>(result.orderId)},
...
void handleWebSocketSubscribe(const std::string& jsonMessage, ix::WebSocket& webSocket, TradingRuntime* runtime) {
    ...
    try {
        ...
        runtime->submitSubscribe(symbol, true);
